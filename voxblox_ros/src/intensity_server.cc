...
//      "intensity_image", 1, &IntensityServer::intensityImageCallback, this);
  ...
  radiation_sensor_sub_ = nh_private_.subscribe(
      radiation_sensor_topic, 1, &IntensityServer::radiationSensorCallback, this);
//  radiation_sensor_sub_ = nh_private_.subscribe(
//      radiation_sensor_topic, 1, IntensityServer::radiationSensorCallback);

}

float IntensityServer::distance(int x, int y) {
  return std::sqrt(x*x + y*y);
}

float IntensityServer::squared_distance(int x, int y) {
  return x*x + y*y;
}
...
void IntensityServer::radiationSensorCallback(
